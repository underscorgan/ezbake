...
(defn get-timestamp-string
  []
  (lein-main/info "GETTING A TIMESTAMP OH NOES")
...
(defn generate-git-tag-from-version
  ...
  {:pre [(string? lein-version)]
   :post [(string? %)]}
  (lein-main/info "in generate-git-tag-from-version, timestamp is set to " timestamp)
...
(defn generate-package-release-from-version
  ...
  {:pre [(string? lein-version)]
   :post [(string? %)]}
  (lein-main/info "in generate-package-release-from-version, timestamp is set to " timestamp)
  (if (.endsWith lein-version "-SNAPSHOT")
    (format "%s.%s"
...
(defmethod action "stage"
  ...
  (let [deployed-version (if (and (deputils/snapshot-version? (:version lein-project))
    ...
    (let [dependencies    (deputils/get-dependencies-with-jars lein-project)
      ...
      (lein-main/info "timestamp is set to " timestamp)
      (lein-main/info "sleeping for a minute")
      (Thread/sleep 60000)
      (lein-main/info "current timestamp is " (get-timestamp-string))
