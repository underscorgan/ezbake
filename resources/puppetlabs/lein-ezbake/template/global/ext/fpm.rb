...
if options.sources.empty?
  options.sources = case options.operating_system
                    ...
                    when :debian, :ubuntu
                      if options.dist == 'trusty' || options.dist == 'wheezy' #trusty and wheezy are the only sysv debs we build
                        ['etc', 'opt', 'var']
                      else
                        ['etc', 'lib', 'opt', 'usr', 'var']
                      end
...
#deb specific options
  ...
  if options.dist == 'trusty' || options.dist == 'wheezy'
    options.java = 'openjdk-7-jre-headless'
  end
